...
<html lang="en">
	...
	<head>
		<base href="../../">
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}}|{{__('adminMessage.About')}}</title>
							...
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									...
									<div class="kt-subheader__breadcrumbs">
										...
										<a href="javascript:;" class="kt-subheader__breadcrumbs-link">{{__('adminMessage.Aboutlisting')}}</a>
								...
								<div class="kt-subheader__toolbar">
									...
									<div class="btn-group">
										<a href="{{url('gwc/About/create')}}" class="btn btn-brand btn-bold"><i class="la la-plus"></i>&nbsp;{{__('adminMessage.createnew')}}</a>
										<!--<button type="button" class="btn btn-brand btn-bold dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
										</button>
										<div class="dropdown-menu dropdown-menu-right">
											<ul class="kt-nav">
												<li class="kt-nav__item">
													<a href="{{url('gwc/About/pdf')}}" class="kt-nav__link">
									...
									<table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
                                        ...
                                        @foreach($AboutLists as $AboutList)
											<tr class="search-body">
												<td>{{$p}}</td>
												<td>{!! $AboutList->title_en !!}</td>
												<td>{!! $AboutList->title_en !!}</td>
												<td>
                                                @if($AboutList->image)
                                                <img src="{!! url('uploads/About/thumb/'.$AboutList->image) !!}" width="40">
                                                @endif</td>
												<td>
                                                <span class="kt-switch"><label><input value="{{$AboutList->id}}" {{!empty($AboutList->is_active)?'checked':''}} type="checkbox"  id="About" class="change_status"><span></span></label></span>
                                                </td>
												<td>{!! $AboutList->created_at !!}</td>
												<td>{!! $AboutList->updated_at !!}</td>
                                                <td class="kt-datatable__cell">
                                                 ...
                                                 <div class="dropdown-menu dropdown-menu-right">
                                                 <ul class="kt-nav">
                                                 <li class="kt-nav__item"><a href="{{url('gwc/About/'.$AboutList->id.'/edit')}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">{{__('adminMessage.edit')}}</span></a></li>
                                                 <li class="kt-nav__item"><a href="javascript:;" data-toggle="modal" data-target="#kt_modal_{{$AboutList->id}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-trash"></i><span class="kt-nav__link-text">{{__('adminMessage.delete')}}</span></a></li>
                                                 </ul>
                                                 </div>
                                                 </div>
                                                 </span>
                                                 ...
                                                 <!--Delete modal -->
 <div class="modal fade" id="kt_modal_{{$AboutList->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										...
										<div class="modal-footer">
											...
											<button type="button" class="btn btn-danger"  onClick="Javascript:window.location.href='{{url('gwc/About/delete/'.$AboutList->id)}}'">{{__('adminMessage.yes')}}</button>
										</div>
									</div>
								</div>
							</div>
                                        ...
                                        @php $p++; @endphp
                                        @endforeach   
                                        <tr><td colspan="8" class="text-center">{{ $AboutLists->links() }}</td></tr>
                                        @else
