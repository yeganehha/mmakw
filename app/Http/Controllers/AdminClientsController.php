...
class AdminClientsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            'email'        => 'nullable|email|min:3|max:150|string|unique:gwc_clients,email',
            'mobile1'      => 'nullable|min:3|max:20',
        ...
        $clients->username=$request->input('username') ?? 'user'.time();
        $clients->password=bcrypt($request->input('password') ?? '123456789' );
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->validate($request, [
            'client_type'  => 'required|max:15|string',
            'name'         => 'required|min:3|max:150|string',
            'email'        => 'nullable|email|min:3|max:150|string|unique:gwc_clients,email,'.$id,
            'mobile1'      => 'nullable|min:3|max:20',
            'website'      => 'nullable|url|string',
            'image'        => 'image|mimes:jpeg,png,jpg,gif,svg|max:2048'
        ]);
